document.addEventListener("DOMContentLoaded", function () {
  ...
  function calcoloGiorni(dataInizio) {
    const dataCorrente = new Date();
    const giornoInizio = new Date(dataInizio);
    const differenza = dataCorrente - giornoInizio;
    console.log(dataCorrente);

    const calcoloGiorniPassati = Math.floor(differenza / (1000 * 60 * 60 * 24));

    const formattaData = new Intl.DateTimeFormat("it-IT", {
      day: "numeric",
      month: "long",
      year: "numeric",
    });
    const dateFormat = formattaData.format(giornoInizio);
    const dataDiOggi = formattaData.format(dataCorrente);
    return { calcoloGiorniPassati, dateFormat, dataDiOggi };
  }
  const dateInizio = "2025-01-24";
  const risultatoGiorno = calcoloGiorni(dateInizio);
  const message = `Data inizio ${risultatoGiorno.dateFormat} giorni passati ${
    risultatoGiorno.calcoloGiorniPassati + 1
  } giorni,
   oggi è ${risultatoGiorno.dataDiOggi}`;
  calcolo.textContent = message;
});
